...
Serial serial;
//Probably change these two values
  ...
  if (input != null) {
    ...
    else if( components.length == 1 )
    {
      if( components[ 0 ] == "stop" ) {
        println( "The scanning operation has stopped" ) ;
        saveStrings("scanning.txt", lines);
      }
    }
    else {
      for( int i=0; i< components.length; i++) {
